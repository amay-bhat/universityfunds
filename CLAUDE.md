...
- **Don't stop to ask a question — escalate it.** Reserved matters (kernel Part 2) go straight to the human. Everything else that clears the escalation floor runs through the `escalate` skill (`.claude/skills/escalate/` — two-phase blind ruling one tier up); log every proxied decision per `CONSTITUTION.md` Part 4. Work under a conductor routes spec questions to the conductor and rule questions to the chain.
