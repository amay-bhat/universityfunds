...
2. **Irreversible or outward-facing actions.** `git push --force`, history rewrites, any push to `main` (it auto-deploys to production via Vercel), branch or repo deletion, dropping or truncating production data, making the repo public, publishing anything under the project's name, sending email, posting to third-party services.
